...
import API from '../../../axios/Api_Fornecedor';
...
const TableFornecedor = () => {
    ...
    const filterData = (searchValue) => {
      const filteredData = mhs.filter(
        (mhs) =>
          mhs.id_fornedor.toLowerCase().includes(searchValue.toLowerCase()) ||
    ...
    const renderData = paginatedData.map((mhs, index) => {
      return (
        <TableFornecedor mhs={mhs} key={mhs.id} refresh={fetchData} stripeRow={stripeRow} index={index} />
      );
    });
          ...
          <Table className='white'>
            <TableHead>
              <TableRow>
                <TableCell>
                  <TableSortLabel
                    active={orderBy === 'id_fornecedor'}
                    direction={order}
                    onClick={() => handleSort('src/Component/Cadastros/Clientes/TableCliente.js')}
                  >
